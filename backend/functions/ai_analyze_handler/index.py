...
- 穴は通し番号で管理する: `# Hole-001: Φ30 Z方向 (X=0, Y=0)` のように番号・径・方向・中心座標を記載
- 穴が貫通穴か止まり穴かもコメントに明記する: 例 `# Hole-001: Φ30 Z方向貫通 (X=0, Y=0)`
...
3. 穴一覧表（Hole-001, Hole-002...の番号・径・方向・貫通/止まり・図面上の位置・CadQuery座標変換後の位置）
...
- 穴は通し番号で管理する: `# Hole-001: Φ30 Z方向 (X=0, Y=0)` のように番号・径・方向・中心座標を記載
- 穴が貫通穴か止まり穴かもコメントに明記する
- すべての数値は意味のある定数として抽出する（マジックナンバー禁止）
- 最終結果を `result` 変数に代入する
- 穴位置は必ず座標変換（図面座標→CadQuery中心原点座標）を適用してから `.center()` で指定する
- 点線は内部形状として解釈する
- show_object() は使用禁止（ヘッドレス実行環境のため）
- `import math` は使用可能
