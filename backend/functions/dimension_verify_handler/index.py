...
[
  {{
    ...
    "confidence": 0.90,
    "ai_reasoning": "図面のΦ6表記と位置寸法から確認。貫通穴であることを他ビューの破線で確認"
  }}
]
```
...
def _assemble_script_template(elements: list[dict]) -> str:
    ...
    # Group holes by orientation for efficient .pushPoints()
    hole_elements = [e for e in feature_elements if e.get("element_type") == "hole"]
    other_features = [e for e in feature_elements if e.get("element_type") != "hole"]

    # Group holes by (orientation, diameter, depth)
    ...
    # Other features (fillets, chamfers, etc.)
