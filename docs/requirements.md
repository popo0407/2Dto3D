...
**バージョン:** 1.0.0  
**作成日:** 2026-03-26  
**ステータス:** ドラフト（BedrockAgent採用方針は §8.3 を参照して協議）
...
- ウイルススキャン: S3アップロードイベントでLambda発火しスキャン（ClamAV on ECS）
...
- WAF: CloudFrontに適用し、SQLi/XSSルール + レートリミット設定
...
[ユーザー]
   │
   ▼
[CloudFront] ←── WAF (Rate Limit / SQLi / XSS)
   │
...
[S3バケット構造]
  ├── 2dto3d-{env}-uploads/    ← 入力図面ファイル
  ├── 2dto3d-{env}-artifacts/  ← 生成STEP/GLTF/STL
  ├── 2dto3d-{env}-previews/   ← AI解析用正規化PNG
  └── 2dto3d-{env}-frontend/   ← React SPA静的ファイル
...
| **WAF** | セキュリティ | CloudFrontに適用、L7攻撃対策 |

### 8.3 【要協議】 Bedrock Agent の採用是非

#### 現状分析

本システムの主要パイプラインは以下のステップで構成される：

```
ファイル受信 → 前処理 → AI解析 → コード生成 → CAD実行 → 軽量化 → 検証 → 通知
```

#### Bedrock Agent の制約・課題

| 制約 | 詳細 | 本システムへの影響 |
|---|---|---|
| **レスポンスタイムアウト** | 最大60秒（同期モード） | CAD実行（Fargate）は120秒以上かかる場合があり、**タイムアウト超過リスク** |
| **コスト** | Agent呼び出しには追加のOrchestration Tokenが発生 | 大量リクエスト時のコスト増 |
| **デバッグ難易度** | Agentのステップ挙動はBlackbox化されやすい | 問題発生時の原因調査が困難 |
| **カスタムOrchestration** | Agentの処理順序変更が難しい | Step Functionsほど柔軟でない |

#### 検討代替案（推奨）

```
【推奨構成】
Step Functions（状態機械）+ Lambda（AI呼び出し）+ ECS Fargate（CAD処理）

利点:
  ✅ Lambdaタイムアウト最大15分、Fargateはタイムアウトなし
  ✅ ステップ単位のリトライ・エラーハンドリングが完全制御可能
  ✅ X-Rayで全ステップのトレーシングが即座に確認可能
  ✅ コスト: Step Functions（月25万回まで無料）+ Bedrock使用量のみ
  ✅ 会話履歴をDynamoDBで管理し、任意の過去コンテキストを参照可能
```

#### Bedrock Agent の限定的採用候補

以下のフェーズ**のみ** Bedrock Agent の採用が検討できる：

```
対話型修正チャット（UC-03）:
  - ユーザーとの往復が短い（≤60秒で処理完結）
  - Tool use（DynamoDB参照・コード更新）が必要
  - セッション管理をAgentに委任できる
  
ただし、DynamoDBとの統合にInline Agentが必要で実装コストが増すため、
まずは Lambda + Bedrock InvokeModel + DynamoDB で実装し、
スケール課題が発生したタイミングでAgent化を検討するのが現実的。
```

#### ➡️ アクションアイテム

**確認事項（要協議）:**
1. チャット修正の1回あたりの応答に60秒以上かかるユースケースを許容するか？
2. Bedrock Agentの持つセッション管理機能（MemoryRetention）の利便性を優先するか？
3. コスト vs 開発工数のトレードオフをどう評価するか？

**現時点の推奨:** 初期リリースはStep Functions + Bedrock InvokeModel を採用。対話修正はLambda + DynamoDB会話履歴で実装。スケール・機能要件を見てAgentへの移行を再検討する。
...
│   │   ├── parse_handler/
│   │   ├── ai_analyze_handler/
│   │   ├── cadquery_runner/     # ECS Fargate コンテナ
│   │   ├── optimize_handler/
│   │   ├── validate_handler/
│   │   ├── chat_handler/
│   │   ├── history_handler/
...
| 環境 | 説明 | AI | Fargate |
|---|---|---|---|
| `dev` | 開発・テスト | `useMockAI=true` (デフォルト) | 無効（Lambda simで代替） |
| `dev --context useMockAI=false` | Bedrock統合テスト | Claude Sonnet 4.6実機 | 無効 |
...
| Bedrock Agent化 | 対話修正フェーズのAgent移行（§8.3参照） | Low |
...
## 付録 B: CadQuery実行環境（ECS Fargate）設計
...
*本ドキュメントは初期要件定義草案です。§8.3（Bedrock Agent採用是非）の協議結果をもとに更新してください。*
