...
## React コンポーネント生成ルール（Pencil UI 編集前提）

本プロジェクトのすべての React UI コンポーネントは、Pencil による編集・カスタマイズを想定して生成されます。

### Pencil 編集対応の原則

- **マークアップ構造の保持**: セマンティック HTML と WAI-ARIA 属性を保持し、Pencil での編集対象を明確にする
- **Tailwind CSS による スタイリング**: インラインスタイルではなく、クラス名ベースでスタイルを指定（Pencil 互換性確保）
- **コンポーネントの粒度**: 再利用可能な単位で分割し、Pencil での部品交換が容易な構造
- **Props 明示化**: すべての Props を TypeScript インターフェースで明示し、Pencil での値の入力が可能な状態

### Pencil テンプレート互換性チェック

- [ ] すべての要素に適切なセマンティック HTML タグを使用しているか（`button`, `input`, `label` 等）
- [ ] `className` は文字列で指定し、条件付きクラスは `clsx` で統合しているか
- [ ] Props はシンプルで型安全（TypeScript）か
- [ ] テキスト内容はハードコードではなく Props で受け取れるか
- [ ] 動的スタイルは Tailwind ユーティリティクラスのみを使用しているか
